...
_Elaborate on your learnings here in `src/exercise/07.md`_

[why-react-needs-a-key-prop](https://epicreact.dev/why-react-needs-a-key-prop/):
_actually_ explains the why/how of `key` prop:

The other links are useless and just dance around, restating the error message
in essence, _saying_ they are explaining it, but without EXPLAINING it, all they
do is restate the solution/issue/error message.

---

In the extra-credit (a demo, no code writing): Click on an item to focus it.
Notice that as the items are shuffled.. the focus properly tracks the item,
_only if_ a _proper_ `key` property was set!

The same is true for \*any **state\*** that exists in the element/component.

For example, try highlighting an item.<br /> The highlighting is maintained when
a _proper_ (immutatable) `key` _is_ provided.<br /> The highlighting is _lost_
when a `key` was not provided (or an improper `key`, like a mutatable `key`,
such as an array index, or a non-unique value was supplied to the `key`, _was_
provided).<br />The same will be true for any _user_ state in custom components.

-- Notice that in the absence of providing a `key`, React internally uses the
`index` as a faux-`key`.<br /> But that only tracks the _location_ (index) in
the new vs old array.<br /> It does not know if the data inside that array item
was mutated, OR if items were added/removed from the array. So, React assumes no
items were added or removed, but instead that the data inside the item was
edited. But it also assumes that all the other items remain the same, so does
not update them. ? Not sure if I have this quite right. Reread the link above:
[why-react-needs-a-key-prop](https://epicreact.dev/why-react-needs-a-key-prop/)
